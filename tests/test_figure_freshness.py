...
- **Most non-census PDFs.** Thirty committed figures come from other
  ...
  the other 25 are not, and seventeen of those 25 `FIGURES.yaml` marks
  ...
  `fig36_fractionation` and `fig37_chemotherapy` read COMMITTED
  ...
  by a thousand left the suite green. The PDFs are still not byte-gated here. Eight of the seventeen are drawn by `generate_figures.py` from
  ...
  check and not sufficient for one. The other nine are not blocked that way, and the first
  ...
  needs anyway, being one of the nine below -- would bring it under a
...
def _spell(n: int) -> str:
    ...
    words = {0: "no", 1: "one", 2: "two", 3: "three", 5: "five", 7: "seven",
             ...
             17: "seventeen", 29: "twenty-nine", 30: "thirty"}
