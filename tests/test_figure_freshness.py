...
- **Most non-census PDFs.** 41 committed figures come from other
  ...
  the other 36 are not, and 28 of those 36 `FIGURES.yaml` marks
  ...
  `fig47_adc_bystander_reach` and `fig48_chemo_decomposition` read COMMITTED
  ...
  by a thousand left the suite green. The PDFs are still not byte-gated here. Eight of the 28 are drawn by `generate_figures.py` from
  ...
  check and not sufficient for one. The other 20 are not blocked that way, and the first
  ...
  needs anyway, being one of the 20 below -- would bring it under a
