...
- **Most non-census PDFs.** 42 committed figures come from other
  ...
  the other 37 are not, and 29 of those 37 `FIGURES.yaml` marks
  ...
  `fig47_adc_bystander_reach` and `fig48_chemo_decomposition` and `fig49_cart_independence` read COMMITTED
  ...
  by a thousand left the suite green. The PDFs are still not byte-gated here. Eight of the 29 are drawn by `generate_figures.py` from
  ...
  check and not sufficient for one. The other 21 are not blocked that way, and the first
  ...
  needs anyway, being one of the 21 below -- would bring it under a
