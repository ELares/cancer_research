...
- **Most non-census PDFs.** 37 committed figures come from other
  ...
  the other 32 are not, and 24 of those 32 `FIGURES.yaml` marks
  ...
  `fig43_sonodynamic_frequency` and `fig44_pdt_fluence_rate` read COMMITTED
  ...
  by a thousand left the suite green. The PDFs are still not byte-gated here. Eight of the 24 are drawn by `generate_figures.py` from
  ...
  check and not sufficient for one. The other 16 are not blocked that way, and the first
  ...
  needs anyway, being one of the 16 below -- would bring it under a
