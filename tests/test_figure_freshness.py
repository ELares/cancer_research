...
- **Most non-census PDFs.** 33 committed figures come from other
  ...
  the other 28 are not, and 20 of those 28 `FIGURES.yaml` marks
  ...
  `fig39_adoptive_escalation` and `fig40_oncolytic_bind` read COMMITTED
  ...
  by a thousand left the suite green. The PDFs are still not byte-gated here. Eight of the 20 are drawn by `generate_figures.py` from
  ...
  check and not sufficient for one. The other 12 are not blocked that way, and the first
  ...
  needs anyway, being one of the 12 below -- would bring it under a
