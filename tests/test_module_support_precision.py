...
def test_the_stated_precision_tracks_the_measurement():
    """The number in the prose must match what comention-regression measured."""
    txt = DOC.read_text()
    ...
    measured = json.loads(REG.read_text())["after"]["weighted"]
    assert abs(int(m.group(1)) / 100 - measured) < 0.02, (
        f"doc says {m.group(1)}%, comention-regression measured "
        f"{100*measured:.1f}%")
...
def test_small_counts_are_flagged_where_they_carry_the_argument():
    ...
    for line in txt.splitlines():
        ...
        if m and int(m.group(1).replace(",", "")) < 50:
            assert "too few to survive" in line, (
