...
A separate final check against numerical-source freeze `91ca37aae10e2f22c19d0c6c4a2923bc4220b61c` again reproduced all three selected development runs exactly. `final_candidate_verification.json` records that commit and the final candidate source hash; `verify_final_candidate.py.txt` preserves the exact verification script. The earlier verification file and its hashes remain unchanged. This confirms implementation agreement with the selected development algorithm, not independent validation.

The two gzip archives preserve the original JSON bytes and all 18 pilot/proposal/result records; gzip time is zero and its filename field is empty. `metadata.json` distinguishes hashes of the unchanged original repository files from hashes of the exact scratch scripts. The scripts are stored as `.txt`, including their historical absolute paths, for provenance. Original source hashes alone do not describe the in-memory substitutions. The candidate verification and scratch mathematical checks are also preserved.
