...
Issue #343's acceptance asks to benchmark "against PhysiCell (via the existing FFI) and/or a published tumor reaction-diffusion model." This report delivers the comparison vs the exponential proxy (above) and the validation vs the closed-form 1-D analytical reaction-diffusion solution (PR 1, a published-model result for the single-source geometry). The **PhysiCell / BioFVM cross-check** of the 3-D multi-vessel field is now **delivered (#408)**: the same matched scenario is solved independently by BioFVM (PhysiCell's LOD/Thomas microenvironment solver) and the two fields agree, identical shape (log-field Pearson r 0.9997 to 1.0, single and two-source), identical effective decay length (both solvers, 55-59 µm; the Yukawa point-source shortening this benchmark flags is reproduced by BOTH, confirming it is real geometry), and a magnitude ratio that converges to 1 as BioFVM's time step dt falls (2.82 to 1.08 at dt 0.02 to 0.001), the residual being BioFVM's LOD operator-splitting error, not a disagreement. See `analysis/calibration/rd-biofvm-crosscheck.md` (driver + build at `simulations/calibration/biofvm/`, harness `scripts/validate_rd_vs_biofvm.py`). Honest status: solver verified against the analytical model, benchmarked against the in-repo proxy, AND cross-checked against an independent external simulator.
