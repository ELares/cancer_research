...
8. **The news layer and the PubMed corpus layer must not overlap.** Journal publishers host both primary research (e.g., `nature.com/articles`, `science.org/doi`, `nejm.org/doi`) and editorial/news content (e.g., `nature.com/news`, `science.org/news`, `jwatch.org`). Only the editorial/news paths belong in `SOURCE_TIER_DEFINITIONS`. Primary research paths are PubMed-indexed and belong in the corpus layer. The `config.py` tier definitions intentionally exclude ambiguous journal paths to prevent issue #99 from classifying peer-reviewed papers as news-tier sources. If a URL from a journal domain does not match a known editorial path prefix, it should be routed to corpus processing, not the news pipeline.

9. **The config is a first-pass subset of the full framework.** The criteria document (Section 2) lists 31 named sources across three tiers. The machine-readable `SOURCE_TIER_DEFINITIONS` in `config.py` encodes a subset — sources behind paywalls that block automated access (e.g., The Cancer Letter) and sources requiring manual URL discovery (e.g., university press offices) are omitted. The criteria document is authoritative; the config is a starting point for issue #99's automated pipeline.

10. **This framework is for the cancer research manuscript specifically**. It does not claim to be a general-purpose news credibility framework.
