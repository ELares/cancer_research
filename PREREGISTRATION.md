...
**P24. An oncolytic front does not cross a tumour below a site-percolation threshold in the permissive fraction, and that threshold is the lattice's own constant.**
- *Quantitative model output:* on a 60³ grid with 26-neighbour spread, at certain transmission the front crosses between a permissive fraction of **0.08** and **0.1**, bracketing the published 26-neighbour site-percolation constant **0.0976** and excluding the 6-neighbour value 0.3116. Below it the front reaches ≤117 µm; above it ~540 µm, the whole tumour. The threshold rises as transmission falls (0.1 → 0.12 → 0.3 at transmission 1/0.6/0.3), the signature of site-bond percolation.
- *Falsification threshold:* the prediction fails if the measured crossing bracket does not contain the lattice's own site-percolation constant, if it contains BOTH constants (in which case it discriminates nothing), or if the threshold does not rise as transmission falls — the last would mean the process is not percolating and the agreement is coincidence.
- *Why it is not circular.* The comparator is from computational physics, not oncology, and nothing here was fitted to it. The engine's own `oncolytic::front_speed` is `2√(D·r)`, the Fisher-KPP speed in a HOMOGENEOUS medium, which has no permissive-fraction term at all and is positive on **19** of the conditions where the spatial front does not cross.
- *Note:* this is a claim about the spread RULE on its own lattice, not about tumours. The permissive fraction is a parameter and nothing in this project measures what share of a real tumour a virus can enter; real receptor expression is clustered, and clustering moves a percolation threshold, so the constant would not transfer even if the fraction were known. Rows where no enterable cell existed near the seed are marked as never-seeded rather than reported as non-crossing — a failure to start is a different claim. Full accounting in `analysis/calibration/oncolytic-percolation-validation.md`.

**P25. A bystander payload multiplies an antibody-drug conjugate's kill most where the antibody penetrates worst — but only if the payload travels beyond the conjugate's own front.**
- *Quantitative model output:* at a payload reach of 2 cells, total kill with a cleavable linker over the same run with a non-cleavable one is **4.92×** at an antibody penetration of 20 µm, falling monotonically to **3.24×** at 320 µm. At a reach of 1 cell it is FLAT — 2.07× to 1.99×, a span of 1.06× across the same sixteenfold penetration range.
- *Falsification threshold:* the prediction fails if the fold advantage does not fall by at least 1.4× across a sixteenfold penetration range at any payload reach greater than one cell, or if it falls at a reach of one cell — the second would mean the effect does not depend on the payload outrunning the conjugate, which is the mechanism claimed.
- *Why it is not circular.* `adc::bystander_kill_fraction` returns the SAME value at every penetration length, because `neighbours_in_reach` is a scalar input with no term for penetration at all. Each point here is a matched PAIR of runs differing only in the linker, so the bystander's contribution is a measured difference rather than a supplied parameter.
- *Note:* the ORDERING is the claim, not the magnitudes. This arm's fitted verdict is NO TARGET and stays there — no published bystander dose-response exists to fit — so the kill values are illustrative. The absolute bystander COUNT mostly rises over the same range, because a better-penetrating conjugate leaves more payload sources behind; both columns are reported because publishing only the ratio would publish the more flattering of two true answers. The payload's reach is a parameter and nothing measures how far a released payload actually travels; the conjugate's distribution is the radial-depth proxy, not a real antibody distribution shaped by binding-site consumption. Full accounting in `analysis/calibration/adc-bystander-validation.md`.

**P26. The depth gradient in a chemotherapy kill separates into a delivery term and a cell-cycle term, and only the second depends on the agent class.**
- *Quantitative model output:* with neither term active the kill is FLAT with depth in all three classes (rim and core within 10%). With delivery held uniform, a quiescent core leaves the phase-nonspecific alkylator **78%** of its core kill, the S-phase agent **43%** and the M-phase agent **39%** — a factor of 1.8. With the cell cycle held uniform, the delivery term alone leaves under 0.5% of the core kill in every class.
- *Falsification threshold:* the prediction fails if the null case (neither term) already shows a depth gradient above 10% — the decomposition would then be decomposing the zoning — or if the cell-cycle term costs the phase-nonspecific agent as much as the phase-specific ones (within 1.3×), which would mean the second term is the drug field under another name.
- *Why it matters and why it needs a geometry:* the two terms have OPPOSITE remedies. If delivery binds, the answer is more drug in; if the cell cycle binds, more drug buys nothing and the answer is recruitment into cycle. A point model has no "deep" for either effect to happen in and cannot pose the question.
- *Note:* WHICH term dominates is a statement about these parameters, not about chemotherapy — the penetration length and potency were chosen, not measured, and a longer length shifts the balance toward the cycle term. What is claimed is the SEPARABILITY and the class-dependence. The cell cycle is READ from radial phenotype, not simulated: nothing divides in this engine, so there is no redistribution between phases and no recruitment — which is precisely the intervention this finding points at and cannot yet evaluate. The fitted verdict stays NO TARGET: this arm's dose-response target was already recorded as unreachable rather than unfitted, and the phase sensitivities are conventional rather than measured. Full accounting in `analysis/calibration/chemo-decomposition-validation.md`.

**P27. Multiplying CAR-T's barrier fractions is correct only where the barriers are independent, and misstates the kill by up to a fifth where they are not.**
- *Quantitative model output:* replacing the `infiltration` SCALAR with a depth field of the same mean, the spatial kill agrees with the product of fractions to **0.6%** when antigen expression is independent of position. At an antigen–depth correlation of −0.6 (antigen deep, where no effector reaches) the product predicts **1.20×** the measured kill; at +0.4 (antigen at the rim) **0.89×**.
- *Falsification threshold:* the prediction fails if the spatial run and the product disagree by more than 3% at ZERO correlation — that is a double-counted barrier, not geometry — or if the divergence away from zero stays inside ±10%, in which case the independence assumption costs nothing and the product is fine as written.
- *Why the control is the design:* the two models are constructed to differ in DISTRIBUTION and nothing else, so agreement at zero correlation is what licenses reading disagreement elsewhere as geometry. The first version of this measurement multiplied the depth field on top of the infiltration fraction instead of in place of it and its control failed at every correlation, which is how the defect was found.
- *Note:* this is NOT a measurement of any real antigen–depth correlation; nothing in this project measures whether antigen expression tracks depth in tumours, or in which sign. The claim is that the answer depends on it. Every barrier value remains a placeholder and this arm's fitted verdict, earned on a different quantity, is untouched. The effector field is the radial-depth proxy, while real T-cell infiltration is shaped by chemokine gradients, exclusion by the surrounding matrix and vessel adhesion, none of which this represents. At the strongest positive correlation the effect SATURATES rather than reversing, because the per-cell antigen probability is clamped at 1; that row is marked. Full accounting in `analysis/calibration/cart-independence-validation.md`.

**P28. Checkpoint blockade's fold benefit is the same whatever it is combined with, and its share of the kill is not — so the treatment generating the most immunogenic signal is the worst partner for it.**
- *Quantitative model output:* on a cold-to-hot ladder, blockade raises immune killing **2.5×** against a pharmacologic inducer and **2.5×** against a physical one — the same, because a multiplier does not know what it multiplies. The immune SHARE of the kill differs **31×** (3.98% against 0.13% unblocked). An untreated tumour gains **exactly zero** at every blockade strength tested. The physical arm releases **4.1× more** danger signal and has the SMALLER share.
- *Falsification threshold:* the prediction fails if the fold benefit differs by more than 1.25× between the two active treatments (the multiplier would then depend on its partner), if the immune share differs by less than 10× (the denominator would not distinguish them), or if the higher-DAMP treatment shows the LARGER share (the ranking inference would then be correct after all).
- *Why it needs a denominator:* `checkpoint::residual_brake` returns a multiplier and a point model has no immune-share to divide by. "Blockade raises immune killing 2.5-fold" is true and does not say 2.5 times what.
- *Note:* the absolute immune contribution in this engine is small — a fraction of a percent of the kill under the physical arm — and that is a known property of the model, not a claim about patients: Section 8.4 records the immune ratio collapsing ~104:1 to 4:1 in three dimensions. Counts are tens of kills and no interval is quoted; CONVERGENCE was checked instead (150 and 250 steps agree within the per-cell RNG) before the numbers were reported. Brake parameters are placeholders and this arm's fitted verdict stays INADMISSIBLE — its response band constrains a product of two unidentifiable factors. The ladder is three points, not a dose-response: it separates cold from warm from hot and does not locate a threshold. Full accounting in `analysis/calibration/checkpoint-priming-validation.md`.

**P29. On a real vessel network the surviving perivascular volume exceeds what a one-vessel sleeve radius predicts, and the shortfall grows with vessel density.**
- *Quantitative model output:* the same applicator on the same cells and the same vessels, under two rules -- cooling from every vessel against cooling from the nearest only. At inter-vessel spacings of 600 µm and wider the two AGREE exactly (the control: there the non-nearest factors are ~1 and the analytic model is correct). At 400/300/250 µm the one-vessel model under-states the survivors **3.29×/4.55×/3.55×**.
- *Falsification threshold:* the prediction fails if the two models disagree by more than 2% at a spacing where vessels are several cooling lengths apart — that would be a bug, not physics — or if the maximum under-statement over the non-degenerate rows is under 2×, in which case P20's radius needs no qualification.
- *Relation to P20:* this QUALIFIES P20 and does not refute it. Its direction stands — thermal ablation fails near vessels, electroporation does not — but its registered radius is a **lower bound** on where recurrence can sit, and the shortfall grows with vessel density, which is the setting where perivascular recurrence is reported.
- *Note:* rows where the all-vessel model leaves the tumour ENTIRELY intact are excluded, not averaged: the ratio drifts back toward 1 there for a reason that is total failure rather than agreement, and a ratio across a saturated arm is not a comparison. The multiplicative compounding of independent heat sinks is a MODELLING CHOICE, not a bioheat solve; a Pennes solve over the same network would be the check and is not done. Vessels are points, where real vessels are tubes that cool a line. The cooling length remains the placeholder P20 already flags, which is why the RATIO between two models sharing it is the result and the millimetres are not. This arm CANNOT be run on the engine's default 20 µm grid at all — the tumour is then smaller than the cooling length and nothing ablates at any density — so it declares a 0.25 mm cell, as `SlabConfig` does for the same reason. Fitted verdict stays UNCONSTRAINED. Full accounting in `analysis/calibration/ablation-superposition-validation.md`.

### Honesty clause for P9 to P29
