...
| Version | ferroptosis-core 0.67.0 (see `simulations/ferroptosis-core/Cargo.toml` for the authoritative current value) |
...
- **In-vitro kill-switch calibration (#330):** the single-cell RSL3 kill switch is fit to CTRPv2 GPX4-inhibitor median dose-response (fit on ML162 RMSE 0.05, held-out validated on the ML210 compound RMSE 0.07 — same CTRPv2 dataset and GPX4-inhibition mechanism, i.e. a held-out *compound*, not cross-platform or cross-mechanism validation; the default in-vivo-tuned switch is ~11x worse, far too RSL3-resistant for in-vitro data). This anchors the single-cell switch to an independent in-vitro dataset with held-out-compound comparison; it does NOT calibrate the in-vivo / spatial layers (see `analysis/calibration/kill-switch-calibration.md`).
