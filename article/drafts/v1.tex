...
\caption{Publication volume by mechanism (2015--2025).}
\label{fig:fig1}
\end{figure}
...
\caption{Mechanism $\times$ cancer type article count matrix.}
\label{fig:fig2}
\end{figure}
...
\caption{Ferroptosis, ICD, and GSH/GPX4 pathway engagement by physical modality ($\chi^2 = 97.3$, $p < 5.9 \times 10^{-23}$).}
\label{fig:fig3}
\end{figure}
...
\caption{Molecular pathway engagement by physical modality (normalized \%).}
\label{fig:fig4}
\end{figure}
...
\caption{Literature disconnect between persister-ferroptosis and SDT-ferroptosis communities.}
\label{fig:fig5}
\end{figure}
...
\textbf{Results} (Figure~\ref{fig:fig7}, Table~\ref{tab:sim}):

\begin{table}[ht]
\centering
\caption{Monte Carlo ferroptosis simulation results (n = 1,000,000 cells per condition). All baselines $<$2\%. Sensitivity analysis: 22/22 parameter perturbations preserve the qualitative result.}
\label{tab:sim}
\begin{tabular}{lcccc}
\toprule
...
\caption{Monte Carlo simulation: ferroptosis death rate by cell phenotype and treatment (n=1M cells/condition). All baselines $<$2\%. Sensitivity: 22/22 holds.}
\label{fig:fig7}
\end{figure}
...
\begin{table}[ht]
\centering
\caption{Ferroptosis engagement across physical modalities (PubMed, March 2026). PDT dominates SDT $\sim$3:1 on every metric.}
\label{tab:modality}
\begin{tabular}{lccl}
\toprule
\textbf{Modality} & \textbf{Ferroptosis articles} & \textbf{Ferroptosis + ICD} & \textbf{Tissue depth} \\
\midrule
...
\caption{Evidence depth along the SDT ferroptosis-ICD chain.}
\label{fig:fig6}
\end{figure}
