...
The DAMP quality per dead cell is not equal across treatments. The autocatalytic lipid peroxidation cascade continues post-threshold for high-ROS treatments: SDT/PDT drive LP to approximately 2$\times$ the death threshold (LP $\sim$20 vs threshold 10) because exogenous ROS sustains the chain reaction, while RSL3's slower LP accumulation barely exceeds the threshold (LP $\sim$10.5). This is modeled as a treatment-specific LP overshoot multiplier (SDT/PDT: 2.0$\times$, RSL3/Control: 1.05$\times$), an estimated parameter based on the autocatalytic propagation kinetics (Biology2e Ch.7-8). The DAMP differential therefore comes from both kill \textit{density} (SDT kills $\sim$140K cells vs RSL3's $\sim$163) and kill \textit{quality} (each SDT dead cell releases $\sim$2$\times$ more DAMPs).
