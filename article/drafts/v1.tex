...
\textbf{Third, CAP is a speculative adjacent lane, not a co-equal conclusion.} CAP broadens the class of externally imposed ROS therapies, but its current literature is sparse and best interpreted as hypothesis-generating for accessible or locoregional settings.

\textbf{Fourth, the strongest version of the PDT/SDT claim is now conditional.} Physical ROS modalities are most plausible when residual cells are locally targetable, when spatial selectivity matters more than systemic reach, and when ferroptosis-related vulnerability is expected to persist long enough for intervention. Outside those conditions, the rationale weakens quickly.
