...
Caveats: the O2 model treats SDT/PDT as fully O2-independent, which is conservative (the Type II photodynamic mechanism requires O2 for singlet oxygen generation). The Fenton reaction substrate (H2O2) is also O2-derived via the superoxide$\rightarrow$SOD pathway, but we modulate only basal_ros, not Fenton rate directly. The O2 field is steady-state (no consumption dynamics) and uses distance from the tumor edge as the O2 source (no explicit vasculature). All of these simplifications mean the actual hypoxic protection is likely STRONGER than what we report.
