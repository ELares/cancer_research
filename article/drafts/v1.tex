...
The past decade has witnessed a substantial diversification of cancer therapeutic mechanisms. Where oncology was historically dominated by the triad of surgery, chemotherapy, and radiation, the contemporary landscape encompasses at least 19 mechanistically distinct approaches ranging from bioelectric modulation and frequency-based physical therapies to CRISPR-mediated gene editing, personalized mRNA cancer vaccines, and microbiome manipulation \cite{shiravand2022_35621637,stine2022_34862480,rojas2023_37165196}. This expansion has been accompanied by a fundamental conceptual shift: the recognition that cancer's hallmark heterogeneity, adaptive resistance, and microenvironmental complexity render single-mechanism cures unlikely for most malignancies, and that rational multi-modal combinations may be required to achieve durable responses \cite{zhu2021_34579759}.

Several developments have accelerated this convergence. The success of immune checkpoint inhibitors, beginning with the landmark approvals of ipilimumab, pembrolizumab, and nivolumab, established that mobilizing endogenous anti-tumor immunity could produce durable remissions even in previously intractable cancers \cite{shiravand2022_35621637}. Yet response rates to single-agent checkpoint blockade remain modest across many tumor types, with primary and acquired resistance affecting the majority of patients \cite{rimassa2023_36933770}. This has driven systematic exploration of combination strategies: checkpoint inhibitors with oncolytic viruses \cite{shalhout2023_36631681}, with CAR-T cell therapy \cite{steffin2025_39604730,peng2024_39134804}, with metabolic reprogramming \cite{chelakkot2023_36768924}, and with physical modalities such as tumor-treating fields (TTFields) \cite{rominiyi2021_33144698,chen2022_35199647} and irreversible electroporation \cite{unknown2019_31130474}.

Simultaneously, the maturation of platform technologies -- lipid nanoparticles for mRNA delivery \cite{chen2022_35969778}, CRISPR-Cas9 for precision gene editing \cite{dimitri2022_35303871,carnevale2022_36002574}, and antibody-drug conjugates for targeted payload delivery \cite{zhou2025_39614090} -- has created a combinatorial toolkit of extraordinary breadth. The question confronting the field is no longer whether multi-mechanism approaches are desirable, but rather which specific combinations, for which cancers, with what evidence, and where the most consequential gaps lie.

Despite this imperative, to our knowledge, no systematic effort has mapped the full mechanism-cancer-evidence landscape across the breadth of therapeutic modalities now under investigation. Individual reviews have addressed specific domains -- immunotherapy combinations \cite{zhu2021_34579759}, nanomedicine \cite{guo2025_40303342,sabit2025_40457437}, oncolytic virotherapy \cite{ma2023_36402738}, or TTFields \cite{rominiyi2021_33144698} -- but a cross-cutting synthesis that identifies convergence patterns, translational bottlenecks, and unexplored opportunities across the entire therapeutic spectrum has been lacking.
...
\caption{Publication volume by mechanism (2015--2025). The 14-fold increase from 134 articles (2015) to 1,911 (2025) reflects the diversification of therapeutic mechanisms, with a marked inflection in 2020 coinciding with mRNA platform maturation and broadening checkpoint immunotherapy experience.}
...
\caption{Mechanism $\times$ cancer type article count matrix. Rows (19 mechanisms) and columns (22 cancer types) are sorted by total article count. Immunotherapy--lung (313) and TTFields--glioblastoma (384) are the dominant pairs. Gray crosses mark zero-count cells.}
\label{fig:heatmap}
\end{figure}
...
Electrochemical therapy (EChT), dominated by irreversible electroporation (IRE), is represented by 422 articles across 15 cancer types, with 14 articles classified at the Phase III tier (though, as with TTFields, this count includes reviews and meta-analyses discussing Phase III data alongside primary trial reports). Pancreatic cancer (180 articles) and prostate cancer (100 articles) are the primary applications. A foundational paper introduced the concept of tissue ablation through irreversible electroporation \cite{davalos2005_15771276} (1,241 citations), and subsequent clinical development has demonstrated safety and feasibility in locally advanced pancreatic cancer \cite{martin2015_26258317} (400 citations) and localized prostate cancer \cite{scheltema2016_27150293,scheltema2018_29594551}.
...
The strongest convergence axis is oncolytic virus-immunotherapy (307 articles), consistent with the mechanistic rationale that viral oncolysis generates immunogenic cell death that may synergize with checkpoint blockade \cite{shalhout2023_36631681,ma2023_36402738}. Combinations with CAR-T (64 articles) and nanoparticle delivery (24 articles) are emerging.
...
\caption{Ferroptosis, ICD, and GSH/GPX4 pathway engagement by physical modality (as \% of each modality's articles). SDT is a statistical outlier ($\chi^2 = 97.3$, $p < 5.9 \times 10^{-23}$ vs all other physical modalities combined for ferroptosis engagement).}
\label{fig:ferro_comparison}
\end{figure}
...
\caption{Molecular pathway engagement by physical modality (normalized \%). SDT uniquely engages the ferroptosis pathway (7\%) and the GSH/GPX4 axis (14\%) while also having the highest ROS generation literature (56\%). TTFields, IRE, and HIFU show 0\% ferroptosis engagement.}
\label{fig:mol_overlap}
\end{figure}
...
\caption{Literature disconnect between the persister-ferroptosis and SDT-ferroptosis research communities. Left: community sizes within our corpus. Right: cross-community article overlap matrix showing zero shared articles between persister-ferroptosis and SDT-ferroptosis literatures.}
...
\subsection{What Our Analysis Adds to Known Biology}

We must be precise about what is already known, what is emerging, and what our analysis contributes.

\textbf{Established knowledge.} Ferroptosis as a cancer vulnerability is well-recognized \cite{lei2022_35338310} (2,045 citations). SDT-triggered ferroptosis via ROS and GSH depletion is documented in dozens of nanoparticle engineering papers. ICD following ferroptosis is recognized. The general concept that resistance tradeoffs create new vulnerabilities is not novel — synthetic lethality, collateral sensitivity (Pluchino et al., 2012), and adaptive therapy (Gatenby \& Brown, 2009) all operate on versions of this principle.

\textbf{Emerging findings we did not discover.} Critically, the connection between therapy-resistant persister cells and ferroptosis sensitivity has been independently validated by others during the period of our analysis. A 2026 Science Advances study demonstrated that cancer persister cells surviving oncogene-targeted therapies are sensitized to ferroptosis, with FSP1 downregulation as a key mechanism \cite{higuchi2026_41481741}. BRAFi-resistant melanoma has been shown to be ferroptosis-vulnerable through AhR-dependent mechanisms \cite{berra2026_41872128}. A broader PubMed search reveals approximately 44 papers connecting OXPHOS, ferroptosis, and resistance — far more than the 4 our corpus captured, reflecting the limitations of our search query design rather than a genuine literature void. Our initial characterization of this connection as a "blind spot" was overstated; it is better described as an emerging research front that our systematic analysis independently converged upon.

\textbf{What our analysis specifically contributes:}
...
| \textbf{IRE} | \textbf{15} | emerging | Invasive (needles) |
| \textbf{HIFU} | \textbf{3} | minimal | Deep (cm) |
| \textbf{TTFields} | \textbf{0} | \textbf{0} | Surface arrays |
...
The mechanism is shared: sensitizer-activated ROS $\rightarrow$ GSH depletion $\rightarrow$ GPX4 inactivation $\rightarrow$ ferroptosis $\rightarrow$ DAMP release $\rightarrow$ ICD \cite{xiong2021_34027953,xu2021_33408790,dong2021_34655115,ren2021_34646381}. SDT-specific data shows 72 GSH/GPX4 articles (11.7\% of corpus) and 6 dual-pathway articles \cite{ren2021_34646381,liu2024_38064417,liao2025_40490790}. A recent Nature Communications paper demonstrates that IRE can also be combined with ferroptosis via iron-based nanoparticles \cite{li2026_41554743}, further indicating that the ferroptosis-physical modality connection is broader than SDT alone.

\textbf{Critical caveats remain}: Most SDT-ferroptosis data involves engineered nanosonosensitizers \cite{xu2021_33408790,dong2021_34655115}. No clinical study has verified SDT-induced ferroptosis or ICD in human tumors. The PDT precedent — 40 years of development without demonstrating robust ICD-mediated immune synergy in randomized trials — applies as a direct cautionary parallel.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../figures/fig6_sdt_chain_evidence.pdf}
\caption{Evidence depth along the SDT $\rightarrow$ ferroptosis $\rightarrow$ ICD chain. Each bar represents the number of SDT articles (out of 617 total) engaging each mechanistic step. The chain is supported at each link but thins substantially at GPX4 inactivation (9 articles), identifying the weakest evidential link.}
\label{fig:chain}
\end{figure}
