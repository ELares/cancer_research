...
- **Book-format manuscript (~115 pp)** with 12 chapters, 3 appendices, and 29 figures (~61,000 words), cross-referenced against all analysis outputs and indexed in [`FIGURES.yaml`](FIGURES.yaml)
...
three modules (`reaction_diffusion`, `spheroid`, `vasculature`) mention neither
ferroptosis nor a physical-ROS modality anywhere in their text. And the `1` is
...
| `article/drafts/` | Manuscript (v1.md + v1.tex) with 29 figures; [`FIGURES.yaml`](FIGURES.yaml) indexes 34 entries, the extra 4 being supplementary and 1 orphan |
