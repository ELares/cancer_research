...
An [archive-only diagnostic](docs/CORRELATED_PROPOSAL_DIAGNOSTICS.md) decomposes
those feature errors and measures conditional precision without new study runs.
...
The [passive 3D immune report](analysis/immune-measurement-report.md) now records
death/release events and actual living-cell eligibility without changing the
...
The [20-block follow-up](docs/IMMUNE_2D_REPLICATION_RESULTS.md) now preserves the
positive activation-exposure contrast across widely spaced seed blocks: mean
SDT − RSL3 **0.555834**, with a whole-block bootstrap interval **[0.554223, 0.557439]**,
in accumulated activation steps per initial tumor cell. No eligible opportunity
...
- **13 Rust simulation binaries**, a mechanistic claim-testing engine covering
  **ten treatment arms plus an untreated control**: ferroptosis induction
  ...
  3,817, so the newer arms are roughly 2.4 to 3.0 times smaller — measured in
...
- **Book-format manuscript (~270 pp at 6x9 trim)** with 12 chapters, 3 appendices, and 47 figures (~82,600 words), cross-referenced against all analysis outputs and indexed in [`FIGURES.yaml`](FIGURES.yaml)
...
| `article/drafts/` | Manuscript (v1.md + v1.tex) with 47 figures; [`FIGURES.yaml`](FIGURES.yaml) indexes 52 entries, the extra 5 being 4 supplementary and 1 orphan |
