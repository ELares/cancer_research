...
- **13 Rust simulation binaries**, a mechanistic claim-testing engine covering
  **nine treatment arms plus an untreated control**: ferroptosis induction
  (RSL3), photodynamic and sonodynamic therapy, ionizing radiation, checkpoint
  blockade, adoptive cell therapy, oncolytic virus, thermal and electrical
  ablation, and antibody-drug conjugates. Around them sit single-cell and
...
- **Book-format manuscript (~270 pp at 6x9 trim)** with 12 chapters, 3 appendices, and 32 figures (~64,900 words), cross-referenced against all analysis outputs and indexed in [`FIGURES.yaml`](FIGURES.yaml)
...
|---|--:|--:|--:|
| committed analyses | 20 | **1** | 124 |
| preregistered predictions | **8 of 14** | 0 | — |
...
five modules (`adoptive`, `oncolytic`, `reaction_diffusion`, `spheroid`,
`vasculature`) mention neither ferroptosis nor a physical-ROS modality anywhere
in their text — and two of those five are modality arms added since, which is
the count moving for the right reason. And the `1` is
...
8 of 14 preregistered predictions, and 32 of 37 modules of the simulation engine, concern ferroptosis or the physical-ROS modalities (PDT and
...
8 of 14 rather than the 8 of 8 it read for most of this project's life. None of
...
| `article/drafts/` | Manuscript (v1.md + v1.tex) with 32 figures; [`FIGURES.yaml`](FIGURES.yaml) indexes 37 entries, the extra 4 being supplementary and 1 orphan |
