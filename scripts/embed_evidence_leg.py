...
def write_report(r):
    ...
    lines = [
        ...
        "",
        "Evaluated on the **v2** gold set (" + str(r["n"]) + " records, 32% positive / 183",
        ...
        f"from {_pct(mh['recall'])} to {_pct(me['recall'])}. Precision goes {_pct(mh['precision'])} ->",
        f"{_pct(me['precision'])}: it does NOT drop, because the k-NN's own precision exceeds the",
