...
def assemble(d: dict) -> dict:
    ...
    sample_pmids = {str(r["pmid"]) for r in d["sample"]}
    adj = {pmid: row for pmid, row in _adjudication().items()
           if pmid in sample_pmids}
    out["adjudicated_n"] = len(adj)
    # Reassembly can receive stored derived fields from a different sample.
    # Its hand-read labels must belong to the sample actually being reported.
...
def render(d: dict) -> str:
    ...
    if not d["organ_toxicity_articles"]:
        L.append(
            f"Generated by `scripts/census_normal_tissue.py` over "
            f"{d['ferroptosis_articles']:,} input articles carrying the "
            "`Ferroptosis` descriptor. No organ-toxicity descriptors matched "
            "those articles. This input supplies no normal-tissue harm "
            "comparison or adjudicated direction.\n"
        )
        return "\n".join(L)
    if not d["adjudicated_n"]:
        L.append(
            f"Generated by `scripts/census_normal_tissue.py`. "
            f"{d['organ_toxicity_articles']:,} of "
            f"{d['ferroptosis_articles']:,} input articles carrying the "
            "`Ferroptosis` descriptor also carry an organ-toxicity descriptor. "
            "No adjudications match this input's sampled PMIDs; biological "
