...
- The "binding-site barrier" half of #335. The model has NO binding-site-barrier
  ...
  SATURABLE: perivascular target binding sequesters drug until they saturate at higher
  dose, making penetration depth dose-dependent. Validating that requires a saturable
  (Michaelis-Menten) uptake term the model does not implement. We document this as a
  model gap; it is the remaining #335 leg, not something the current model can validate.
...
def run(args):
    ...
    result = {
        ...
        "binding_site_barrier_note": ("The model has no binding-site barrier: uptake is LINEAR "
                                      ...
                                      "and needs a Michaelis-Menten uptake term the model lacks. Adding it "
                                      "is a separate future model EXTENSION (a new mechanism), not a "
                                      "refinement of this linear model; documented as a gap, not validated."),
...
{r['binding_site_barrier_note']} Issue #335 asks to validate the binding-site barrier;
