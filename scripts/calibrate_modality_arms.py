...
def _immunotherapy_arm() -> dict:
    ...
    return {
        ...
        "diagnosis": (
            ...
            "**This row stays INADMISSIBLE and a DIFFERENT target has since "
            "succeeded on one axis.** A RATIO between two strata of one trial "
            "cancels the response-to-kill mapping exactly, so KEYNOTE-158's "
            "29% against 6% by tumour mutational burden constrains the "
            "antigenicity SHAPE where this absolute band constrained nothing "
            "-- 31% of the scanned shape grid survives it. That is a separate "
            "target with its own artifact "
            "(`analysis/calibration/checkpoint-validation.md`), it identifies "
            "neither factor, and it does not make THIS fit admissible: the "
            "verdict here is about the ORR band, and the ORR band is still "
            "unable to separate a product."),
