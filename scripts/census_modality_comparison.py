...
def render(d: dict) -> str:
    ...
    if not any(r["mesh_articles"] or r["text_articles"] for r in d["rows"]):
        L.append(
            f"Generated by `scripts/census_modality_comparison.py` over "
            f"{d['census']:,} census records. No compared modalities matched "
            "either the descriptor or text arm. Regulatory maturity and "
            "descriptor validity cannot be compared in this input.\n"
        )
        return "\n".join(L)
    L.append(
        f"Generated by `scripts/census_modality_comparison.py` over "
