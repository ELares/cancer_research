...
def assemble(d: dict) -> dict:
    ...
    out["thin_arm"] = (arm2["role"] if out["arm1_precedent"] > out["arm2_precedent"]
                       else arm1["role"] if out["arm2_precedent"] > out["arm1_precedent"]
                       else None)
    out["cell_line_rows"] = [
...
def render(d: dict) -> str:
    ...
    if not d["arm1_precedent"] and not d["arm2_precedent"]:
        L.append(
            f"Generated by `scripts/census_protocol_precedent.py` over "
            f"{d['ferroptosis_articles']:,} input articles carrying the "
            "`Ferroptosis` descriptor. No named protocol reagents matched "
            "either arm, so their precedent cannot be compared in this input.\n"
        )
        return "\n".join(L)
    if d["thin_arm"] is None:
        L.append(
            f"Generated by `scripts/census_protocol_precedent.py` over "
            f"{d['ferroptosis_articles']:,} input articles carrying the "
            f"`Ferroptosis` descriptor. The two protocol arms each have "
            f"{d['arm1_precedent']:,} matched mentions; neither arm has "
            "less precedent in this input.\n"
        )
        return "\n".join(L)
    L.append(
    ...
    factor = (f"a factor of {d['arm_asymmetry']}" if d["arm_asymmetry"] is not None
              else "no finite ratio because arm 2 has zero matched mentions")
    L.append(
        f"**Arm 1 ({d['arm1_precedent']:,} articles) against arm 2 "
        f"({d['arm2_precedent']:,}) — {factor}.** The "
