...
figs = {
    ...
    '24': ('fig27_resistance_asymmetry', 'The resistance-mechanism asymmetry (2D model, flagship). Under each tumor-microenvironment resistance mechanism, pharmacologic RSL3 collapses while physical SDT holds. (a) Hypoxia: RSL3 overall kill falls from 3.7\\% to 0.1\\% (normoxic to hypoxic) while SDT holds 91.9\\% to 87.8\\%. (b) Stromal/CAF and (c) acidic pH are measured as OXYGENATED-RIM kill, because in the hypoxic core RSL3 is already floored (${\\sim}0.1\\%$) so the rim is where CAF/pH suppression of RSL3 is observable: RSL3 drops (1.5\\% to 1.2\\% under CAF, 1.5\\% to 0.9\\% under acidosis) while SDT is barely affected (95.5\\% to 94.4\\% / 95.6\\%). (d) Immune/ICD coupling: SDT produces 521 ICD-driven immune kills versus 5 for RSL3 (104:1 in 2D). \\textbf{Confidence tiers differ per panel} (titles): the hypoxia leg is the most contested (SDT is modeled O$_2$-independent, an optimistic upper bound, Section 7.1) and the 2D immune ratio over-extrapolates (${\\sim}4{:}1$ under 3D volumetric dilution). Magnitudes rest on uncalibrated biochemistry; the cross-modality direction, not the numbers, is the result.'),
}
