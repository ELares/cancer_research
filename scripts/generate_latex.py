...
figs = {
    ...
    '11': ('fig14c_class_by_site', 'Mechanism class by anatomical site. Enrichment is a site\'s share of class site assignments divided by its share of all census site assignments. Overlapping sites count separately; totals do not measure unique-article coverage. Bold labels mark class enrichments straddling 1.0 under this chosen baseline; these flags depend on the baseline and do not rule out indexing or descriptor differences. The physical class omits radiotherapy, its largest real member.'),
    '12': ('fig15c_mechanism_pairs', 'The ten most frequent mechanism pairs in the census. Counts record articles carrying both descriptor groups; they do not establish tested combinations. Co-tagging fractions depend on label definitions, tagged-article denominators and the selected population.'),
    ...
    '14': ('fig17_damp_heatmap', 'Terminal DAMP spatial distribution after immune coupling, baseline run (O$_2$ gradient $\\lambda$=120$\\mu$m, no stromal shielding, no pH gradient; per-panel scaling---intensity not comparable across panels). SDT covers the full tumor area (139,640 kills, 521 immune kills); RSL3 produces sparse isolated hotspots (163 kills, 5 immune kills). This terminal field is not the earlier DAMP exposure of living cells eligible for immune killing; post-horizon additions cannot explain earlier immune kills.'),
    ...
    '16': ('fig19_immune_coupling_flow', 'Immune coupling pathway: SDT produces dense kill and a broad modeled DAMP field; RSL3 produces sparse kill and localized signal. Both paths scale DAMP release by post-death LP. The 521 versus 5 immune kills give a 104:1 total immune-kill ratio, not a DAMP potency or per-dead-cell immunogenicity ratio. The passive baseline replay finds no eligible opportunity at or above Kd=50; this diagram does not establish a saturation or density mechanism (Section 8.2).'),
