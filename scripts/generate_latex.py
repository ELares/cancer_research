...
figs = {
    ...
    '30': ('fig34_depth_reach', 'How far each modality reaches into tissue. (a) Kill at the surface and at 9.4 mm: photodynamic therapy falls from 92.5\\% to 0.6\\% while megavoltage radiation goes 45.4\\% to 43.7\\%, and the pharmacologic arm does not attenuate at all. (b) Retention as a share of surface kill, which RANKS THE UNTREATED ARM FIRST at 400\\% -- a ratio of two near-zero numbers is not robustness, and the two panels must be read together or neither. Attenuation is fixed physics; every kill magnitude rests on uncalibrated biochemistry.'),
    '31': ('fig35_calibration_verdicts', 'What a published target could settle, per arm. Drawn because the informative rows are the ones that failed: checkpoint blockade is inadmissible because its response band constrains a product of two factors neither of which is identifiable from it, and the antibody-drug-conjugate bystander effect has no published target to fit at all. ADMISSIBLE means a parameter reproduces a band, not that the arm is validated -- every arm here feeds no number this manuscript reports.'),
}
