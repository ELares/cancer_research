...
def match_diagnostic_therapy_links(text: str) -> list[str]:
    ...
    for chain_id in DIAGNOSTIC_THERAPY_ORDER:
        ...
        has_diagnostic = any(kw.lower() in text for kw in chain["diagnostic"])
        has_feature = any(kw.lower() in text for kw in chain["feature"])
        has_intervention = any(kw.lower() in text for kw in chain["intervention"])
