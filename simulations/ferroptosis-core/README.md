...
| `phenotype_mufa` | Phenotype-specific SCD1/MUFA accumulation rates (#363): `PhenotypeMufaConfig` (per-phenotype multiplier on the global `scd_mufa_rate`; `identity()` default) + `apply_phenotype_mufa_rates_3d` set each tumor cell's per-cell `Cell::mufa_rate` (`None` ⇒ global ⇒ byte-identical), so the acute-vs-established MUFA build-up (#339) gets a phenotype-specific time constant instead of one shared rate. DIRECTION is genuinely uncertain (MUFA enrichment resists ferroptosis, Magtanong 2019 PMID 30686757, and SCD1 protects, Tesfay 2019 PMID 31270077, but persisters are also GPX4-dependent/vulnerable, Hangauer 2017 PMID 29088702), so `literature()` is an UNCALIBRATED placeholder. Not in the C ABI. Off-by-default identity ⇒ byte-identical |
