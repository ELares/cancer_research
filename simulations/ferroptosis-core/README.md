...
| `checkpoint` | Checkpoint blockade (#837): receptor occupancy, PD-L1 as a brake modifier kept separate from tumour mutational burden as an antigenicity modifier, interferon-driven adaptive PD-L1 induction, the three resistance modes (primary, adaptive, acquired), and a dimensionless response index with a RATIO function -- the ratio cancels the unknown response-to-kill mapping, which is what made a stratified trial usable where an absolute response rate was not. |
...
| `reaction_diffusion` | Steady-state reaction-diffusion supply field (#343): `reaction_diffusion_solve` / `reaction_diffusion_supply_field` + `ReactionDiffusionConfig` + `analytical_1d_slab`. Solves the linear steady-state PDE `D∇²c − k·c = 0` over the same vessel network as `vasculature::vessel_supply_field`, with vessel Dirichlet sources (`c=1`), tumor-only consumption (`γ = k·h²/D = (h/λ)²`), and finite-volume no-flux domain boundaries, by deterministic SOR (fixed linear-index Gauss-Seidel sweep). The vasculature/oxygen proxies are monotonic in distance-to-nearest-source; a real irregular network superposes all nearby vessels while consumption draws the field back down, so the steady state has non-monotonic pockets the proxy averages away. The diffusion-consumption length `λ = sqrt(D/k)` equals the proxy's `lambda_um`, so a single isolated source reproduces `exp(−dist/λ)` (the proxy is the single-source limit) — any divergence is the genuine multi-vessel/consumption effect. `analytical_1d_slab` is the standard closed-form 1-D reaction-diffusion cosh profile (diffusion-limited-distance lineage after Thomlinson-Gray 1955; the cosh form is the generic textbook solution, not from that paper) that the unit tests use as an **analytical self-consistency check** — the SOR discretization converges to the exact solution of its own continuous equation (max abs error < 0.01). The genuine external benchmark (proxy-vs-RD field/kill + a PhysiCell/published-PDE cross-check) and the proxy-adequacy docs are the remaining #343 criteria, delivered in the sim-tme-3d wiring (PR 2). Drop-in alternative supply field; nothing in the production matrix calls it ⇒ byte-identical |
| `acsl4` | ACSL4-status biomarker stratification (#444): ACSL4 ligates the PUFA ferroptosis requires into membranes, so a tumor's ACSL4 status sets its oxidizable-PUFA baseline — ACSL4-high sensitive, ACSL4-negative intrinsically refractory (Doll 2017 PMID 27842070). `pufa_boost_from_status` + `Params.acsl4_status_boost`; `status_from_zscore` bridges a cBioPortal mRNA z-score (#462). Off-by-default identity ⇒ byte-identical |
| `alox` | ALOX lipoxygenase-isoform-specific peroxidation rate + MCFA→ACSL4 PUFA sensitization (#446): `AloxConfig` collapses an isoform/expression mix to `alox_propagation_boost` (ALOX15-high ⇒ faster propagation) + `mcfa_pufa_boost`. Uncalibrated placeholders; off-by-default identity ⇒ byte-identical |
| `copper` | Copper-ionophore / ferroptosis-cuproptosis crosstalk (#485): elesclomol/disulfiram overload intracellular Cu, depleting GSH and degrading GPX4 to enable both death modes, gated by ATP7B efflux (Gao 2021 PMID 34390123). `CopperConfig` + `gsh_retention`/`gpx4_retention`; `disabled()` ⇒ byte-identical |
| `ifngamma` | IFN-γ → System Xc⁻ ferroptosis-sensitization, the molecular RETURN arm of the immune loop (#443): activated-CD8 IFN-γ represses SLC7A11/SLC3A2 → cystine starvation → more ferroptosis (Wang 2019 PMID 31043744). `IFNGammaConfig` + `system_xc_retention` + `acsl4_upregulation`; `disabled()` ⇒ byte-identical |
| `repair` | ESCRT-III membrane-repair brake on death EXECUTION (#465): a cell whose lipid peroxide already crossed `death_threshold` can be resealed (Ca²⁺→CHMP5/6) for a finite per-cell budget, so more repair ⇒ more resistance (Dai 2020 PMID 31761326). `escrt_rescue`/`escrt_can_attempt`; off-by-default ⇒ byte-identical |
| `senescence` | Therapy-induced-senescence ferroptosis program (#341): `SenescenceConfig` + `apply_senescence_program_3d` mark a fraction of tumor cells and scale iron/defenses — BIDIRECTIONAL (resist upstream cystine/iron triggers vs senolytic under direct GPX4 inhibition) — plus a signed SASP→immune coupling (cell-autonomous `sasp_immune_mult` + the diffusing `sasp_field`). Off-by-default identity ⇒ byte-identical |
| `trigger_wave` | Propagating ferroptotic trigger-wave front (#482): a 1-D bistable Nagumo reaction-diffusion front, `front_speed`/`analytical_front_speed` (`c = sqrt(D·k/2)·(1−2a)`, `k~iron` ⇒ `c~sqrt(iron)`), calibrated to Co 2024's measured speeds (5.52 µm/min baseline, iron-tunable 2.33/9.40; PMID 38987590). Opt-in ⇒ byte-identical |
...
All ~40 simulation parameters are documented with literature sources and sensitivity ratings in [`parameter_provenance.md`](https://github.com/ELares/cancer_research/blob/main/simulations/calibration/parameter_provenance.md) (in the parent repository's `simulations/calibration/` directory).
