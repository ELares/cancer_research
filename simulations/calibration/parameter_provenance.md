...
Sobol total-effect screening at the Persister+RSL3 operating point (see `analysis/sobol-sensitivity-report.md`) ranks how much the single-cell kill rate constrains each biochemical rate constant. Parameters the kill rate is insensitive to (total-effect ST < 0.05) are NOT constrainable from kill-rate calibration and are marked here so they are not read as data-fitted:

| Parameter | ST | constrainable from kill rate? |
|---|--:|:--:|
| `lp_propagation` | 0.658 | yes |
| `gpx4_rate` | 0.330 | yes |
| `gsh_scav_efficiency` | 0.021 | NO (kill-rate-insensitive) |
| `fsp1_rate` | 0.019 | NO (kill-rate-insensitive) |
| `gsh_km` | 0.014 | NO (kill-rate-insensitive) |
| `fenton_rate` | 0.006 | NO (kill-rate-insensitive) |
| `death_threshold` | 0.002 | NO (kill-rate-insensitive) |
