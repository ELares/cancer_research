...
| Tissue drug penetration | `drug_transport` | **Uncalibrated (illustrative)** | Exponential Krogh-cylinder approximation across 3 tissue types; all transport parameters are estimated, so the cross-tissue comparison (manuscript §6.1) is directionally informative, not quantitative. 2D helper, not part of the sim-tme-3d track. |
